<template>
  <div class="page-wrapper">
      ...
      <div class="detail-hero">
        <div class="detail-hero-inner">
          ...
          <div class="detail-info fade-in-up">
            ...
            <p class="detail-desc">
              {{ course.description || '의료기관 도입 검토를 위한 장비 상세 정보입니다.' }}
            </p>

            <div class="detail-meta">
              <span>제조·공급사: {{ displayInstructorName }}</span>
              <span>도입 기관: {{ displayEnrollmentCount }}곳</span>
            </div>
          </div>
    ...
    <div v-else class="loading-center">
      <p class="empty-text">장비 정보를 불러오지 못했습니다.</p>
    </div>
  </div>
</template>
...
const categoryConfig = {
  '진단장비': { badge: 'badge-teal', bg: 'thumb-teal', thumb: 'spring_boot' },
  '환자 모니터링': { badge: 'badge-teal', bg: 'thumb-teal', thumb: 'vue_js' },
  '수술장비': { badge: 'badge-blue', bg: 'thumb-blue', thumb: 'kubernetes' },
  '검사장비': { badge: 'badge-purple', bg: 'thumb-purple', thumb: 'python' },
  '디지털 헬스': { badge: 'badge-pink', bg: 'thumb-pink', thumb: 'generative_ai' },
}
...
const buttonLabel = computed(() => {
  if (isInstructor.value) return '공급사 계정은 구매 불가'
  if (enrollmentStatus.value === 'ACTIVE') return '도입 내역으로 이동'
  if (enrollmentStatus.value === 'PENDING') return '구매 신청 · 결제 처리 중'
  return '결제하고 도입하기'
})
...
const helperText = computed(() => {
  if (isInstructor.value) {
    return '제조·공급사 계정은 장비를 구매할 수 없습니다.'
  }

  if (enrollmentStatus.value === 'ACTIVE') {
    return '이미 도입한 장비입니다. 도입 내역에서 확인할 수 있습니다.'
  }

  if (enrollmentStatus.value === 'PENDING') {
    return '구매 신청이 접수되었습니다. 결제 후 도입 내역에서 확인할 수 있습니다.'
  }

  return '결제를 진행하면 장비 도입 신청이 함께 처리됩니다.'
})
...
async function handlePrimaryAction() {
  ...
  if (!course.value?.id) {
    enrollError.value = '장비 정보가 올바르지 않습니다.'
    return
  }

  if (isInstructor.value) {
    enrollError.value = '제조·공급사 계정은 장비를 구매할 수 없습니다.'
    return
  }
  ...
  try {
    ...
    enrollError.value = e.response?.data?.message || '결제 또는 장비 도입 신청에 실패했습니다.'
  } finally {
