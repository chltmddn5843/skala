  ...
  <header class="app-header">
    <div class="header-inner">
      ...
      <router-link to="/" class="logo">
        <img src="@/assets/images/logo/main_logo.png" alt="MediEquip" class="logo-img" />
        <span class="logo-text">MediEquip</span>
      </router-link>

      <!-- 네비게이션 -->
      <nav class="nav-links" v-if="auth.isAuthenticated">
        <router-link to="/courses" class="nav-link" :class="{ active: $route.path.startsWith('/courses') }">의료장비</router-link>
        <router-link to="/enrollments" class="nav-link" :class="{ active: $route.path === '/enrollments' }">도입 내역</router-link>
      </nav>
