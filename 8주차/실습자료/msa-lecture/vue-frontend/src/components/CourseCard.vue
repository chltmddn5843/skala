<template>
  <router-link :to="`/courses/${course.id}`" class="course-card">
    ...
    <div class="card-body">
      ...
      <div class="card-meta">
        <span class="instructor">{{ course.instructorName || '제조·공급사' }}</span>
        <span class="price">₩{{ Number(course.price).toLocaleString() }}</span>
      </div>
      <div class="card-footer">
        <span class="enrolled">도입 기관 {{ course.enrollmentCount?.toLocaleString() || 0 }}곳</span>
      </div>
    </div>
  </router-link>
</template>
...
const categoryConfig = {
  ...
  '수술장비': { bg: 'thumb-blue', badge: 'badge-blue', thumb: 'docker' },
