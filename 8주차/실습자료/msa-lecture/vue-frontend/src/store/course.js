...
export const useCourseStore = defineStore('course', () => {
  ...
  const categories = ['전체', '진단장비', '환자 모니터링', '수술장비', '검사장비', '디지털 헬스']
  ...
  const categoryThumbnailMap = {
    '진단장비': thumbnailMap.SPRING,
    '환자 모니터링': thumbnailMap.VUE,
    '수술장비': thumbnailMap.KUBERNETES,
    '검사장비': thumbnailMap.PYTHON,
    '디지털 헬스': thumbnailMap.AI
  }
  ...
  async function fetchCourses() {
    ...
    try {
      ...
      error.value = e.message || '의료장비 목록을 불러오지 못했습니다.'
      courses.value = []
    } finally {
      loading.value = false
    }
  ...
  async function fetchCourse(id) {
    ...
    try {
      ...
      error.value = e.message || '장비 정보를 불러오지 못했습니다.'
