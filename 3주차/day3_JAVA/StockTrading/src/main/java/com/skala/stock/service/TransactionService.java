...
public class TransactionService {
    ...
    /** 특정 사용자의 특정 주식에 대한 거래 내역을 최신순으로 조회한다 */
    public List<TransactionDto> getUserStockTransactions(Long userId, Long stockId) {
        List<Transaction> transactions =
                transactionRepository.findByUserIdAndStockIdOrderByTransactionDateDesc(userId, stockId);
        return transactions.stream()
                .map(this::convertToDto)      // 리스트의 각 Entity를 DTO로 변환
                .collect(Collectors.toList());
    }
